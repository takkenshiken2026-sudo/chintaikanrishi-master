...
def category_to_cat(col: str) -> str:
    c = norm(col)
    if "賃貸住宅管理業法" in c or "賃管" in c:
        return "lawN"
    if "賃貸借" in c or "民法" in c or "借地借家" in c or "契約" in c:
        return "rightsN"
    if "建物" in c or "設備" in c or "建築" in c:
        return "limit"
    if "会計" in c or "税" in c or "保険" in c:
        return "limit"
    if "不動産" in c and "経営" in c:
        return "lawN"
    return "rightsN"
...
def main() -> int:
    ...
    OUT_JS.write_text(header + json.dumps(out, ensure_ascii=False, indent=2) + ";\n", encoding="utf-8")
