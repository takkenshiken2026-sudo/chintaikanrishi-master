#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate write_chintai_hub_s40-s44_content.py and premium FAQ block."""
...
def _premium_block(rows: list[dict]) -> str:
    ...
    for row in rows:
        ...
        lines.append(f'    "{slug}": [')
        qs = [
            (f"{title}の要点は？", f"{summary}主体・手続・数値を表で整理してください。" + _OFFICIAL),
            (f"{title}の試験引っかけは？", f"{title}では主体逆転・手続省略・数値混同の肢に注意し、用語集と条文で確認してください。" + _OFFICIAL),
            (f"{title}の対策は？", f"{title}の比較表・数値表・誤答パターン表を作成し、過去問を反復してください。" + _OFFICIAL),
            (f"{title}の確認先は？", "用語集と賃貸住宅管理業法・借地借家法・協議会要項を参照してください。" + _OFFICIAL),
        ]
        for q, a in qs:
            if len(a) < 100:
                a = a + _OFFICIAL_TAIL
