...
FIELD_GUIDE_FORBIDDEN_RE = re.compile(
    r"5行表|7行表|5列表|/terms/|terms/|Day3解き直し|Day0→3→7|"
    r"11/22新規0|9月通し\d+/50|9/\d+通し\d+/\d+"
)

# 分野別記事に最低1つは試験論点語が必要
FIELD_GUIDE_SUBSTANCE_RE = re.compile(
    r"条文|論点|制度|法第|借地|借家|契約|権利|義務|規定|敷金|更新|正当事由|保証|賃貸|"
    r"明渡|預り金|返還|重説|原状回復|管理受託|サブリース|分別管理|"
    r"修繕|点検|設備|税務|保険|管理費|"
    r"判例|横断|分野またぎ|"
    r"委託|受託|報酬|媒介|重要事項"
)
...
def validate_field_guide_genre(slug: str, patch: dict[str, str]) -> list[str]:
    ...
    if FIELD_GUIDE_FORBIDDEN_RE.search(combined):
        errors.append(
            ...
            f"(5列表, 7行表, /terms/, 9/6通し 等). Link to study-plan instead."
        )
