...
(function () {
  function boot() {
    ...
    // 早期キューを flush。site-analytics.js のロード前に index.html の gotoPage / popstate から
    // 呼ばれた page_view を、現在 URL（直前の config で送信済み）と重複しないものだけ送出する。
    try {
      var q = Array.isArray(window.__GA4_PV_QUEUE__) ? window.__GA4_PV_QUEUE__ : null;
      window.__GA4_PV_QUEUE__ = [];
      if (q && q.length) {
        var currentPath = (typeof location !== "undefined")
          ? (location.pathname + location.search + location.hash)
          : "";
        for (var i = 0; i < q.length; i++) {
          var item = q[i];
          if (!item) continue;
          var qPath = item[0] != null ? String(item[0]) : "";
          var qTitle = item[1] != null ? String(item[1]) : "";
          if (!qPath || qPath === currentPath) continue;
          try { ga4PageView(qPath, qTitle); } catch (_e4) {}
        }
      }
    } catch (_e3) {}
  }
